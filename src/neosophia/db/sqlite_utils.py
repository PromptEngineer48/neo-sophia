...
def get_tables_from_postgres(conn: psycopg2.extensions.connection) -> List[str]:
    """Get a list of table names from the PostgreSQL database"""
    table_names = []

    with conn.cursor() as cursor:
        cursor.execute("SELECT table_name FROM information_schema.tables WHERE table_schema='public' and table_name='account_opening_balance_entry';")
